...
localSuccSequences :: LocalProtocol -> Graph -> [Sequence]
localSuccSequences loproto gg
  | null (loproto gg) = [ [] | isSolved gg ]
  | otherwise =
      [ [] | isSolved gg ] ++ [ c : rest | c <- loproto gg, rest <- localSuccSequences loproto (call gg c) ]
